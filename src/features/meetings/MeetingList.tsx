...
export default function MeetingList() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 pb-16 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
      ...
      <div className="bg-amber-50/80 dark:bg-amber-900/20 backdrop-blur-sm border border-amber-200 dark:border-amber-700 text-amber-800 dark:text-amber-200 p-3 sm:p-4 rounded-2xl sm:rounded-3xl mb-4 sm:mb-6 flex items-start gap-2.5 sm:gap-3 text-xs sm:text-sm shadow-sm">
        <Info className="w-4 h-4 sm:w-[18px] sm:h-[18px] mt-0.5 shrink-0 text-amber-500 dark:text-amber-400" />
      ...
      <div className="min-h-[300px]">
        {loading ? (
            ...
            {[1, 2, 3].map((i) => (
              ...
              >
                ...
                <div className="space-y-2 flex-1">
                  <div className="h-4 bg-slate-100 dark:bg-gray-700 rounded-md w-1/2" />
          ...
          <div className="flex flex-col items-center justify-center h-[300px] bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-3xl border-2 border-dashed border-slate-200 dark:border-gray-800 shadow-sm px-4">
            ...
            <p className="text-slate-800 dark:text-white font-bold text-lg sm:text-xl text-center">No meetings found</p>
            <p className="text-slate-500 dark:text-gray-400 text-sm sm:text-base mt-1.5 sm:mt-2 max-w-sm text-center font-medium">
              {search
          ...
          <div className="space-y-3 sm:space-y-4">
            {filtered.map((meeting) => (
              <div
                ...
                className="group relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-gray-800 p-4 sm:p-5 shadow-sm hover:border-amber-200 dark:hover:border-amber-600 hover:shadow-lg hover:shadow-slate-100/50 dark:hover:shadow-gray-900/50 hover:-translate-y-0.5 cursor-pointer transition-all duration-300 overflow-hidden"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 mt-1.5 sm:mt-2 text-xs sm:text-sm text-slate-500 dark:text-gray-400 font-medium">
                      <CalendarDays className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 dark:text-gray-500" />
                      {new Date(meeting.createdAt).toLocaleDateString()}
                    </div>
                  </div>

                  <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 w-full sm:w-auto pt-2 sm:pt-0 mt-1 sm:mt-0 border-t border-slate-100 dark:border-gray-800 sm:border-0">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-bold rounded-full border shadow-sm ${
                        meeting.status === 'completed'
                          ...
                          : 'bg-slate-50 dark:bg-gray-800 text-slate-600 dark:text-gray-400 border-slate-200 dark:border-gray-700'
                      }`}
                    >
                      <span
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-3">
                      {isAdmin && (
                        <button
                          ...
                          className="p-1.5 sm:p-2 text-slate-400 dark:text-gray-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors sm:opacity-0 group-hover:opacity-100 sm:translate-x-2 group-hover:translate-x-0"
                          title="Delete meeting"
                        >
