  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 pb-16 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 sm:gap-2 text-slate-500 hover:text-sky-600 font-semibold text-sm transition-colors mb-1 sm:mb-2"
      >
      ...
      {createFormOpen && isAdmin && (
        <div className="bg-white/80 backdrop-blur-sm p-4 sm:p-6 rounded-3xl border-2 border-dashed border-sky-200 shadow-sm mb-6 flex flex-col xl:flex-row items-stretch gap-4 animate-in slide-in-from-top-4 fade-in duration-300">
          <div className="hidden sm:flex w-10 h-10 rounded-2xl items-center justify-center bg-sky-100 text-sky-600 shrink-0 shadow-inner">
      ...
      <div className="relative w-full">
        <div className="absolute inset-0 bg-gradient-to-r from-sky-100/50 to-transparent rounded-2xl blur opacity-0 group-focus-within:opacity-100 transition duration-500 pointer-events-none" />
        <div className="relative flex items-center">
          <Search className="absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-slate-400 group-focus-within:text-sky-500 transition-colors" />
          <input
            type="text"
            placeholder="Search releases..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-3.5 bg-white/60 backdrop-blur border border-slate-200 rounded-2xl text-sm sm:text-base font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-sky-100 focus:border-sky-300 transition-all shadow-sm"
          />
        </div>
      </div>
      ...
      <div className="min-h-[300px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-[300px] bg-white/80 backdrop-blur-sm rounded-3xl border-2 border-dashed border-slate-200 shadow-sm px-4">
            ...
            <p className="text-slate-800 font-bold text-lg sm:text-xl text-center">No releases found</p>
            ...
            {!search && isAdmin && (
              <button
                ...
                className="mt-6 font-bold text-sky-500 hover:text-sky-600 flex items-center gap-1.5 transition-colors"
              >
              ...
              <div
                ...
                className={`group relative bg-white/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl border p-4 sm:p-5 shadow-sm transition-all duration-300 overflow-hidden ${
                  editId === release.id
                    ...
                    : 'border-slate-200/80 hover:border-sky-200 hover:shadow-lg hover:shadow-slate-100/50 cursor-pointer'
                }`}
              >
                {/* Left accent line */}
                <div className="absolute left-0 top-0 bottom-0 w-1 sm:w-1.5 bg-sky-400 scale-y-0 group-hover:scale-y-100 origin-center transition-transform duration-300 rounded-r-sm" />
                ...
                {editId === release.id ? (
                  ...
                  <div className="flex flex-col lg:flex-row gap-3 animate-in fade-in zoom-in-95 duration-200">
                    <input
                      ...
                      className="flex-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300"
                      autoFocus
                    />
                    <input
                      type="text"
                      ...
                      placeholder="Summary"
                      className="flex-1 w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300"
                    />
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setEditId(null)}
                        className="flex-1 lg:flex-none px-3 py-2 text-slate-500 hover:text-slate-800 rounded-lg font-semibold text-sm bg-slate-50 border border-slate-200 lg:border-transparent lg:bg-transparent hover:bg-slate-100 transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        onClick={() => handleUpdate(release.id)}
                        className="flex-1 lg:flex-none px-3 py-2 bg-sky-100 text-sky-700 rounded-lg font-semibold text-sm hover:bg-sky-200 transition-colors"
                      >
                        Save
                      </button>
                    </div>
                  ...
                  >
                    <div className="flex items-start sm:items-center gap-3 sm:gap-4 min-w-0 flex-1">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl flex items-center justify-center bg-gradient-to-br from-sky-100 to-sky-200 text-sky-700 shadow-inner ring-1 ring-white/50 shrink-0">
                        <Layers className="w-4 h-4 sm:w-5 sm:h-5" />
                      </div>
                      <div className="truncate">
                        <h3 className="font-bold text-slate-800 group-hover:text-sky-600 transition-colors text-base sm:text-lg truncate">
                          {release.name}
                        </h3>
                        {release.summary && (
                          <p className="text-xs sm:text-sm text-slate-500 mt-0.5 sm:mt-1 line-clamp-2 whitespace-normal sm:whitespace-nowrap sm:truncate">{release.summary}</p>
                        )}
                        {release._count && (
                          <span className="inline-block mt-1 sm:mt-2 text-[10px] sm:text-xs font-semibold text-slate-400">
                    ...
                    <div className="flex items-center justify-end sm:justify-start gap-3 sm:ml-4 shrink-0 w-full sm:w-auto">
                      ...
                      <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-slate-300 group-hover:text-sky-500 transition-colors" />
                    </div>
                  </div>
                )}
              </div>
