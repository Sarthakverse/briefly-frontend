...
import { getRecentEnhancements } from '../home/homeApi';
...
export default function RecentEnhancements() {
  ...
  const [enhancements, setEnhancements] = useState<RecentEnhancement[]>([]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
      ...
      <button
        ...
        className="group inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-amber-600 transition-colors"
      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 max-w-md">
            The 5 most recent enhancements deployed across all your releases.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
          <span className="text-sm font-semibold text-slate-700">Latest Enhancements</span>
          {!loading && enhancements.length > 0 && (
            <span className="text-xs font-medium text-slate-500 bg-slate-100 px-2 py-1 rounded-md">
              Top {enhancements.length}
            </span>
          )}
        </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 sm:p-12 text-center">
              ...
              <p className="text-slate-500 text-sm max-w-sm mb-6">
                There hasn't been any recent enhancement activity across your releases yet.
              </p>
              <button
                onClick={() => navigate('/adapters')}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold bg-white border border-slate-200 text-slate-700 hover:border-amber-300 hover:text-amber-600 transition-all shadow-sm active:scale-95"
              >
                View Adapters
              </button>
            </div>
            ...
            <div className="divide-y divide-slate-100">
              {enhancements.map((enh) => (
                <div
                  key={enh.id}
                  onClick={() => navigate(`/enhancements/${enh.id}/meetings`)}
                ...
                >
                  ...
                  {/* Icon */}
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-300 bg-slate-100 text-slate-500 group-hover:bg-amber-50 group-hover:text-amber-600 group-hover:shadow-sm">
                    <Zap className="w-5 h-5" strokeWidth={2} />
                  </div>

                  {/* Content */}
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    <h3 className="text-sm font-semibold text-slate-900 group-hover:text-amber-700 transition-colors duration-200 truncate mb-1">
                      {enh.name}
                    </h3>
                    
                    <div className="flex items-center gap-2 sm:gap-3 text-[11px] font-medium text-slate-500 overflow-hidden">
                      <span className="text-slate-700 font-semibold truncate max-w-[100px] sm:max-w-xs">
                        {enh.release?.adapter?.name || 'Unknown Adapter'}
                      </span>
                      
                      <span className="w-1 h-1 rounded-full bg-slate-300 shrink-0" />
                      
                      <span className="truncate max-w-[100px] sm:max-w-xs">
                        {enh.release?.name || 'Unknown Release'}
                      </span>
                      ...
                      {enh.createdAt && (
                        <>
                          ...
                          <span className="hidden sm:flex items-center gap-1.5 whitespace-nowrap">
                            ...
                            {new Date(enh.createdAt).toLocaleDateString(undefined, { 
